rye_hole_noyeast_bar <- bread_vit2 %>% filter(sample == "Hole Rye no yeast") %>%  ggbarplot(x = "component", y = "folate",
  ...
  annotate('text',x = c("PGA", "10-HCO-H2", "10-HCO-PGA", "H4", "5-CH3-H4", "5-HCO-H4","5,10-CH+-H4"), 
           y = c(2, 2, 10, 3, 3, 2, 3), label = c(" ", " ", " ", " ", " ", " ", " "))

#piechart 
rye_hole_noyeast_p1 <- bread_vit_mean2 %>% filter(Group.1 == "Hole Rye no yeast" & Group.2 == "Bread") %>% 
...
rye_hole_noyeast_p2 <- bread_vit_mean2 %>% filter(Group.1 == "Hole Rye no yeast" & Group.2 == "Bread Digesta") %>% 
  ggpie("x",  fill = "Group.3", color = "white",palette = "npg") + 
  ggtitle("Digesta") + theme(plot.title = element_text(hjust = 0.5)) + 
  theme(legend.position = "right")  + theme_void() + theme(legend.title = element_blank())

rye_hole_noyeast_pie <- ggarrange(rye_hole_noyeast_p1, rye_hole_noyeast_p2, ncol = 1, nrow = 2, 
                          common.legend = TRUE, legend = "right")

rye_hole_noyeast <- ggarrange(rye_hole_noyeast_bar, rye_hole_noyeast_pie, widths = c(2, 1), labels = c("K a) Rye, Roll (no added yeast)", "b)"),
