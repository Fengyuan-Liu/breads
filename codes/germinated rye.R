germinated_rye_bar <- bread_vit %>% filter(sample == "Germinated Rye Flat") %>%  ggbarplot(x = "component", y = "folate",
...
germinated_rye_p2 <- bread_vit_mean %>% filter(Group.1 == "Germinated Rye Flat" & Group.2 == "Bread Digesta") %>% 
  ggpie("x",  fill = "Group.3", color = "white",palette = "npg") + 
  ggtitle("Digesta") + theme(plot.title = element_text(hjust = 0.5)) + 
  theme(legend.position = "right")  + theme_void() + theme(legend.title = element_blank())

germinated_rye_pie <- ggarrange(germinated_rye_p1, germinated_rye_p2, ncol = 1, nrow = 2, 
                           common.legend = TRUE, legend = "right")

germinated_rye <- ggarrange(germinated_rye_bar, germinated_rye_pie, widths = c(2, 1), labels = c("G a) Germinated Rye, Square", "b)"),
