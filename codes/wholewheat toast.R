wholewheat_toast_bar <- bread_vit %>% filter(sample == "Whole Wheat") %>%  ggbarplot(x = "component", y = "folate",
  ...
  annotate('text',x = c("PGA", "10-HCO-H2", "10-HCO-PGA", "H4", "5-CH3-H4", "5-HCO-H4","5,10-CH+-H4"), 
           y = c(2, 2, 8.5, 7, 1.5, 5, 5), label = c("*", " ", "*", " ", "**", "**", " "))

#piechart for rye toast
wholewheat_toast_p1 <- bread_vit_mean %>% filter(Group.1 == "Whole Wheat" & Group.2 == "Bread") %>% 
...
wholewheat_toast_p2 <- bread_vit_mean %>% filter(Group.1 == "Whole Wheat" & Group.2 == "Bread Digesta") %>% 
  ggpie("x",  fill = "Group.3", color = "white",palette = "npg") + 
  ggtitle("Digesta") + theme(plot.title = element_text(hjust = 0.5)) + 
  theme(legend.position = "right")  + theme_void() + theme(legend.title = element_blank())

wholewheat_toast_pie <- ggarrange(wholewheat_toast_p1, wholewheat_toast_p2, ncol = 1, nrow = 2, 
                           common.legend = TRUE, legend = "right")

wholewheat_toast <- ggarrange(wholewheat_toast_bar, wholewheat_toast_pie, widths = c(2, 1), labels = c("F a) Whole Wheat, Toast", "b)"),
