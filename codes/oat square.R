oat_square_bar <- bread_vit %>% filter(sample == "Oat Flat") %>%  ggbarplot(x = "component", y = "folate",
  ...
  annotate('text',x = c("PGA", "10-HCO-H2", "10-HCO-PGA", "H4", "5-CH3-H4", "5-HCO-H4","5,10-CH+-H4"), 
           y = c(2, 2, 7, 7, 12, 2, 5), label = c(" ", " ", "", " ", "**", " ", " "))

#piechart for rye toast
oat_square_p1 <- bread_vit_mean %>% filter(Group.1 == "Oat Flat" & Group.2 == "Bread") %>% 
...
oat_square_p2 <- bread_vit_mean %>% filter(Group.1 == "Oat Flat" & Group.2 == "Bread Digesta") %>% 
  ggpie("x",  fill = "Group.3", color = "white",palette = "npg") + 
  ggtitle("Digesta") + theme(plot.title = element_text(hjust = 0.5)) + 
  theme(legend.position = "right")  + theme_void() + theme(legend.title = element_blank())

oat_square_pie <- ggarrange(oat_square_p1, oat_square_p2, ncol = 1, nrow = 2, 
                           common.legend = TRUE, legend = "right")

oat_square <- ggarrange(oat_square_bar, oat_square_pie, widths = c(2, 1), labels = c("E a) Oat, Square", "b)"),
